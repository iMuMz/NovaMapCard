<template>
            ...
            <card class="rounded flex flex-col" :style="'padding:5px;height: ' + this.mapHeight">
...
export default {
    ...
    if (!this.card.height) {
      ...
      if (this.card.type == "GeoJson") {
          iniLocation = latLng(this.card.centerLat,this.card.centerLon)
      } else if (this.card.type == "LatLon") {
          iniLocation = latLng(this.card.latitude, this.card.longitude)
      }
     ...
     if (this.card.type == "GeoJson") {
        ...
        popupName = this.card.popupName
        //console.log(this.card.popupName)
